...
func (a *api) Get(match string, handler faas.HttpMiddleware, opts ...MethodOption) {
	matchPath := path.Join(a.path, match)
	r, ok := a.routes[matchPath]
	if !ok {
		r = a.m.NewRoute(a.name, matchPath)
	}
	r.Get(handler, opts...)
	a.routes[matchPath] = r
}
